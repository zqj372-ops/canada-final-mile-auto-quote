...
def _pack_multi_vehicle(
    ...
    *,
    ...
    if physical and _all_same_floor_unit(physical) and _homogeneous_grid_bound_applies(
        physical[0].unit, profiles
    ):
        capacity = sum(
...
def _homogeneous_grid_bound_applies(
    unit: HandlingUnitInput,
    profiles: Sequence[VehicleProfile],
) -> bool:
    """Return whether the single-orientation grid bound is a proven limit.

    Mixed 90-degree layouts can hold more rectangles than the best
    single-orientation grid (for example four 1x2 dominoes in a 3x3 floor
    beat the 3-unit grid), so the grid count is only a valid upper bound for
    the published standard pallet on the built-in vehicle floors, where the
    operational maxima (14 on 26 ft, 30 on 53 ft) are locked by regression
    tests.  Custom profiles and non-standard units go through the bounded
    search instead of being rejected by this shortcut.
    """

    if unit.length_cm != Decimal("121.92") or unit.width_cm != Decimal("101.60"):
        return False
    if not unit.floor_rotation_allowed:
        return False
    return all(
        profile.code in _BUILTIN_STANDARD_FLOOR_DIMENSIONS
        and (profile.length_cm, profile.width_cm) == _BUILTIN_STANDARD_FLOOR_DIMENSIONS[profile.code]
        for profile in profiles
    )
