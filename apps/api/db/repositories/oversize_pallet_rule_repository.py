...
class OversizePalletRuleRepository:
    ...
    def publish_draft(self, actor: CurrentActor) -> tuple[OversizePalletRuleConfig, int]:
        ...
        # The next version is computed as max(version)+1 without a row lock.
        # Concurrent publishes can race on the same number; the (rule_id,
        # version) unique constraint prevents duplicate rows, so recompute the
        # version in a fresh transaction a bounded number of times before
        # surfacing the conflict to the caller.
        config_json = config.model_dump(mode="json")
        for _attempt in range(_MAX_PUBLISH_RETRIES):
            try:
                ...
                # Another publisher committed the same version number first.
                # Roll back the failed transaction and try a fresh max+1.
                self.session.rollback()
        raise ValueError("规则发布并发冲突,请重试")
    ...
    def admin_snapshot(self) -> dict[str, object]:
        ...
        return {
            ...
            "published_at": self.latest_published_at(),
        }

    def latest_published_at(self) -> str | None:
        """Return the ISO timestamp of the newest published snapshot."""

        latest = self.session.scalar(
            select(OversizePalletRuleVersion.published_at)
            .where(OversizePalletRuleVersion.status == "published")
            .order_by(
                OversizePalletRuleVersion.published_at.desc(),
                OversizePalletRuleVersion.id.desc(),
