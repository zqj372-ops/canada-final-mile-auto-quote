...
export default function QuoteCalculationPanel({
  ...
  const manualRequired = result?.manual_review_required ?? Boolean(parsed.missing_fields.length);
  ...
  return (
    <section className="ai-glass-panel min-w-0 p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        >
          报价可信度 {result?.confidence ?? parsed.confidence}%
        </span>
      </div>

      {manualRequired && (
        <div className="mt-5 rounded-md border border-amber-300/50 bg-amber-300/10 px-4 py-3 text-sm font-semibold leading-6 text-amber-50">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        ...
        <Metric label="计费托数" value={result?.billing_pallets ? `${result.billing_pallets} 托` : "待匹配"} />
      ...
      <div className="mt-5 rounded-lg border border-cyan-300/40 bg-cyan-300/10 p-4">
